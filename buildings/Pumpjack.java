 ...
 * @author Aaron Ng
 * @date Jan 10, 2018
 * Mining Drill that mines oil from oil tiles
 */
public class Pumpjack extends PoweredBuilding {
	...
	public Pumpjack(int health, int level,ArrayList<Item> ores) {
		super(health, level,new Dimension(2,2),0,2);
	...
	public Pumpjack(int health, int level) {
		super(health, level,new Dimension(2,2),0,4);
		this.setDim(new Dimension(2,2));
		this.outputInventory=new Item[4];
		setTimeSinceExtract(System.nanoTime());  
	}
	...
	public void update() {
		...
		for(int i=0;i<ores.size();i++) {
			...
			//If outputInventory isn't null then only add 1 to the stack every 4 seconds	
				...
				outputInventory[i].setStack( outputInventory[i].getStack() + 1);
				timeSinceExtract=timeNow;
			}
		}
	}
