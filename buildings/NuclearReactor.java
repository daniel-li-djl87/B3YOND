 ...
 * @author Aaron Ng
 * @date Dec 18, 2017
 * Nuclear Reactor converts water into steam, superheated water and radioactive waste using uranium
 */
public class NuclearReactor extends Building {
	 ...
	 * @param level
	 */
	public NuclearReactor(int health, int level) {
		super(health, level,new Dimension(3,3),1,3);  //
		...
		recipeBook.add(new Recipe(new Item[] {new Water(2)}, new Item[] {new Steam(4), new SuperHeatedWater(1), new RadioactiveWaste(1)}));
	...
	public class Sorting implements Runnable{
		...
		public void run() {
			...
			if(powerSource instanceof Uranium) {  //different power time for different products
				setPowerTime(5000*((Uranium) powerSource).getStack());
			}else {
				setPowerTime(0);
			}
			...
			while(powerTime > 0) { //Only smelt if the furnace has power left
				...
				for (int j = 0; j < inputInventory.length;j++) { 
					...
					} // End for loop cycling through recipes
