/**
 * IndustrialGradeBattery.java
 * @author Naymul Mohammed
 * @Date December 14, 2017
 * IndustrialGradeBattery object of the game used to store energy
 */
...
public class IndustrialGradeBattery extends PoweredBuilding {
	 ...
	 * Constructor for Industrial battery
	 * @param health, level
	 * @return nothing
	 */
	public IndustrialGradeBattery(int health, int level) {
		super(health, level, new Dimension(1,3),0,0);
		this.setDim(new Dimension(1,3));
		setCapacity(0);
		clock=new Clock();
		clock.update();
	}
	...
	public  void update() {
		...
		clock.update(); //Update time elapsed

		if(getPower()) { //Checks to see if battery is getting charged from somewhere

			this.setCapacity((int) (getCapacity()  +(clock.getElapsedTime()*1000)));  //Increase capacity by 1 second every second
			if(getCapacity()>600000) {
				setCapacity(600000);    //Maximum capacity
			}
			
		}else{ //If battery isn't getting charged then discharge
			if(getCapacity()>0) {  //If capacity greater than 0 discharge
				setCapacity((int) (getCapacity() -(clock.getElapsedTime()*1000)));   //Lower capacity by 1 second every second
			}
