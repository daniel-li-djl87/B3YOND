 ...
 * @author Aaron Ng
 * @date Dec 18, 2017
 * Steam Turbine generates electricity by consuming steam; faster than steam engine
 */
public class SteamTurbine extends PoweredBuilding {
	 ...
	 * @param level
	 */
	public SteamTurbine(int health, int level) {
	...
	public class Smelting implements Runnable{
		...
		public void run() {
			smelting = true;
			while (inputInventory[0] != null && inputInventory[0] instanceof Steam && inputInventory[0].getStack() >= 30) { // Only run steam engine if inventory has enough items

					inputInventory[0].setStack(inputInventory[0].getStack() - 30); // subtract 1 from stack
